...
plugin = Plugin()
addon = xbmcaddon.Addon("plugin.video.userhpo")
pluginrootpath = "plugin://plugin.video.userhpo"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.userhpo/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.userhpo(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.userhpo" in item["path"]:
